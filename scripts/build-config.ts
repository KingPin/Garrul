...
/**
 * Only the four always-needed secrets. Wrangler warns on every name in this
 * list that isn't set, so listing the per-provider OAuth pairs would nag a
...
const buildSecretsRequired = (): string =>
	[
		...
		"# Wrangler validates these on `wrangler dev` and warns when one is unset.",
		"# Only the always-needed secrets are listed: optional per-provider",
		"# credentials would otherwise warn on every install that doesn't use them.",
		"[secrets]",
