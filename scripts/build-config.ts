 ...
 *
 * Two of those come from the `Bindings` type in src/index.ts rather than the
 * registry: KV fields are typed `KVNamespace`, not `string`, so they are not
 * registry entries — the parity assertion in build-manifest.ts requires the
 * registry to be exactly the `string` fields. src/index.ts is already the
 * machine-read source for the manifest's kvNamespaces; the two KV lists join it.
 *
...
const buildKvNamespaceBlocks = (kv: string[]): string =>
	[
		`# Generated by \`${GENERATED_BY}\` from ${BINDINGS_SOURCE}. Do not edit by hand.`,
...
const buildOutputs = (): { outputs: Output[]; problems: string[] } => {
	...
	const { kv } = parseBindings(readBindingsSource(REPO_ROOT));
