...
- **Server** — `src/i18n/`. `tFor(locale)` returns a translator; `t` is the pre-bound English one. API error bodies and email/feed templates go through it. **Never reintroduce a module-global "active locale"** — one Worker isolate serves concurrent requests, so a mutable module-level locale leaks across requests at every `await`. Locale arrives per-request via `c.get("t")` (see `src/lib/locale.ts`).
...
Locale is a property of the **site**, not the reader: `data-lang` → host page `<html lang>` (reviewed locales only) → operator `default_locale` → `en`. `Accept-Language` and `navigator.language` are deliberately not consulted — a German reader on an English blog should see an English widget.
