...
| `comments[].email` | string \| null | `comments.email` | Tombstones keep this in the dump — `delete()` nulls `author` and `website` but leaves `email` alone. The adapter drops it on a tombstone: identity is the name+email seed, so keeping it would mint one ghost per deleted author and re-attach an identity isso had already stripped. Every tombstone lands on the same anonymous ghost instead. |
