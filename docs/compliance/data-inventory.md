...
| `moderator_notes` | `target_id`, `author_id`, `body` (free text) | Internal moderator context on a comment or a user | Legitimate interests | Indefinite; no sweep | **Partly** — notes *about* the erased user are deleted; notes they *authored* about other people are kept, like their audit rows | Yes, as `moderator_notes`, with `author_id` removed and comment-level notes excluded |
